...
{  
  ...
  serial_connection.begin(9600);                                   // Serial connection is for GPS communication at 9600 baud
  ...
  compass.setOffset(101,-43,163);                                  // Adjust Compass Offset
...
// Slider Value for Speed
  ...
  if (blueToothVal)                                    
  {    
     //Serial.println(blueToothVal);                            // for testing purpose
...
void CompassTurnLeft()                                                           // This procedure turns the car 90 degrees to the left based on the current desired heading
{
  ...
  while ( abs(desired_heading - compass_heading) >= compass_dev)                 // If the desired heading is more than Compass Deviation in degrees from the actual compass heading then
    ...
    int z = (y - 360);                                                           // z = y - 360
        ...
        if ((z <= 180) && (z >= 0))                                              // if z is less than 180 and not a negative value then turn Right
            {                                                                    // otherwise turn left
              RightTurn();                             
            } 
            else
